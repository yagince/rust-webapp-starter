...
impl Handler<ArticleId> for ConnDsl {
    ...
    fn handle(&mut self, article_id: ArticleId, _: &mut Self::Context) -> Self::Result {
        ...
        let the_article = article.filter(&id.eq(&article_id.article_id))
            .load::<Article>(conn)
...
pub fn article_list(req: HttpRequest<State>)
...
pub fn article_new(req: HttpRequest<State>)
                   -> Box<Future<Item=HttpResponse, Error=Error>>
{
    req.clone().json()
        .from_err()
        .and_then(move |article_new: ArticleNew| {
            req.state().db.send(
                ...
                .and_then(|res| {
                    match res {
                        Ok(msg) =>
                            Ok(httpcodes::HTTPOk.build().json(msg)?),
                        Err(_) =>
                            Ok(httpcodes::HTTPInternalServerError.into()),
                    }
                })
...
impl Handler<ArticleNew> for ConnDsl {
    ...
    fn handle(&mut self, article_new: ArticleNew, _: &mut Self::Context) -> Self::Result {
        ...
        diesel::insert_into(article).values(&new_article).execute(conn)
            .map_err(error::ErrorInternalServerError)?;
